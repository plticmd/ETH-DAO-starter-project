...
import { ConnectWallet, useActiveChain, useAddress, useContract } from "@thirdweb-dev/react";
...
const Home: NextPage = () => {
  ...
  const editionDrop = useContract("0x969BB10710FD678085aC918D8935FE607f22E327", "edition-drop").contract;

  // トークンコントラクトの初期化
  const token = useContract("0xb114D0D247671b11039D0f745B1a55619Ba30E18", "token").contract;

  // 投票コントラクトの初期化 (VOTE_ADDRESS)
  const vote = useContract("0x137d5e6BDF809780d0fc1043A9c221a33653CFfC", "vote").contract;
...
const memberList = useMemo(() => {
 ...
 //テストネットが Sepolia ではなかった場合に警告を表示
  else if (address && network?.data?.chain?.id !== 11155111) {
    console.log("wallet address: ", address);
